...
//	// Create index buffer
//	bd.Usage = D3D11_USAGE_DEFAULT;
//	bd.ByteWidth = sizeof(WORD) * *NumOfIndice;        // 36 vertices needed for 12 triangles in a triangle list
//	bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
//	bd.CPUAccessFlags = 0;
//	InitData.pSysMem = indices;
//	ThrowIfFailed(g_pd3dDevice->CreateBuffer(&bd, &InitData, &mBuffers.pIndexBuffer));
...
//
//	#define NumOfVertices_Axis 4
//	#define NumOfIndices_Axis 6
//
//	NumOfVertices_Axes = 2 * NumOfVertices_Axis;
//	NumOfIndices_Axes = 2 * NumOfIndices_Axis;
//
//	std::array <SimpleVertex, NumOfVertices_Axis * 2> vertices;
//	std::array <XMFLOAT3, NumOfVertices_Axis> xVertices;
//	std::array <XMFLOAT3, NumOfVertices_Axis> yVertices;
//	std::array <WORD, NumOfIndices_Axis * 2> indices;
//	std::array <WORD, NumOfIndices_Axis> xIndices;
//	std::array <WORD, NumOfIndices_Axis> yIndices;
//
//	float AngleOfArrow = XM_PI / 10.0f;
//	float LengthOfArrow = 0.5f;
//	float a = AxisMin;
//	float b = AxisMax;
//
//	// Fill in x vertices and indices
//	xVertices =
//	{
//		XMFLOAT3{ a, 0.0f, 0.0f},
//		XMFLOAT3{ b, 0.0f, 0.0f},
//		XMFLOAT3{ b - LengthOfArrow, +LengthOfArrow * tan(AngleOfArrow), 0},
//		XMFLOAT3{ b - LengthOfArrow, -LengthOfArrow * tan(AngleOfArrow), 0 }
//	};
//
//	xIndices = 
...
//	};
//
//	// Fill in y vertices and indices
//	UINT k = 0;
//	for (auto s : xVertices)
//	{
//		yVertices[k].x = s.y;
//		yVertices[k].y = s.x;
//		yVertices[k].z = s.z;
...
//	for (auto s : xIndices)
//	{
//		yIndices[k++] = s + NumOfVertices_Axis;
//	}
//
//	// Fill in combines vertices and indices
//	k = 0;
//	for (UINT i = 0; i < NumOfVertices_Axis; i++)
//	{
//		vertices[i] = { xVertices[i], AxesColor };
//		vertices[i + NumOfVertices_Axis] = { yVertices[i], AxesColor };
//	}
//
//	for (UINT i = 0; i < NumOfIndices_Axis; i++)
//	{
//		indices[i] = xIndices[i];
//		indices[i + NumOfIndices_Axis] = yIndices[i];
//	}
//
//	D3D11_BUFFER_DESC bd;
//	ZeroMemory(&bd, sizeof(bd));
//	bd.Usage = D3D11_USAGE_DEFAULT;
//	bd.ByteWidth = sizeof(SimpleVertex) * NumOfVertices_Axes;
//	bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
//	bd.CPUAccessFlags = 0;
//	D3D11_SUBRESOURCE_DATA InitData;
//	ZeroMemory(&InitData, sizeof(InitData));
//	InitData.pSysMem = vertices.data();
//	ThrowIfFailed(g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pAxesVertexBuffer));
//
//	// Create index buffer
//
//
//	bd.Usage = D3D11_USAGE_DEFAULT;
//	bd.ByteWidth = sizeof(WORD) * NumOfIndices_Axes;        // 36 vertices needed for 12 triangles in a triangle list
//	bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
//	bd.CPUAccessFlags = 0;
//	InitData.pSysMem = indices.data();
//	ThrowIfFailed(g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pAxesIndexBuffer));
//}
//
